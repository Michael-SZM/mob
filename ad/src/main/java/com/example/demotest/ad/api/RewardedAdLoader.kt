 ...
 * 具体 SDK 调用全部委托给 [platforms] 适配器，加载成功的 SDK 广告对象被包装为 [IRewardedAd] 协议对象。
 *
 * 生命周期约定：一次加载对应一次展示，广告对象在展示关闭后即失效，
 * 业务方在 [AdEventListener.onAdClosed] 后需重新 [load] 获取新广告（开启 autoReloadOnClose 时自动补位）。
 *
 * 实例请通过 [AdsManager.createRewardedLoader] 创建，以获得门面统一注入的平台与配置。
 */
class RewardedAdLoader internal constructor(
    private val platforms: List<AdPlatform>,
    private val request: RewardedAdRequest,
) {
    ...
    /**
     * 加载激励视频（竞价 × 瀑布流同时执行，比价取最高者入缓存，不绑定展示监听）
     *
    ...
    private fun doLoad(context: Context, targets: List<AdPlatform>, listener: AdLoadListener) {
        AdLogger.i(TAG, "开始加载激励视频: 参与平台=${targets.joinToString { it.name }}（竞价与瀑布流同时执行）")
        AdRaceRunner<IRewardedAd>(targets) { platform, onLoaded, onError ->
            platform.loadRewarded(context, request, onLoaded, onError)
        }.run(
            onSettled = { winner, outcome ->
                AdLogger.i(
                    TAG,
                    "激励视频竞得: platform=${winner.platform.name}, 渠道=${winner.channel.label}, " +
                        "ecpm=${winner.ad.ecpm ?: "无"}, 耗时=${winner.costMs}ms",
                )
                outcome.releaseLosers()
                retryHelper.reset()
                AdCacheManager.put(AdType.REWARDED, winner.ad)
                listener.onAdLoaded()
            },
            onError = { error ->
                AdLogger.w(TAG, "激励视频加载失败: $error")
    ...
    private fun resolveRacePlatforms(listener: AdLoadListener): List<AdPlatform>? {
        ...
        if (configured.isEmpty()) {
            listener.onAdError(AdError(AdError.CODE_EMPTY_AD_UNIT, "激励视频未配置任何平台的广告位 ID（adUnitIds）"))
            return null
        }
    ...
    fun show(
        ...
        val ad = AdCacheManager.take<IRewardedAd>(AdType.REWARDED, strategy)
        if (ad == null) {
            AdLogger.w(TAG, "展示失败：激励视频缓存未命中")
            return false
        }
        AdLogger.i(TAG, "展示激励视频: ecpm=${ad.ecpm ?: "无"}")
        // 委托转发全部展示事件，仅额外拦截 onAdClosed 实现关闭后补位
        ad.setEventListener(
            object : RewardedAdEventListener by listener {
    ...
    fun destroy() {
        retryHelper.reset()
        AdCacheManager.remove<IRewardedAd>(AdType.REWARDED)
    }
